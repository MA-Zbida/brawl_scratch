...
def _load_actions(data: Any) -> np.ndarray | None:
    ...
    elif "actions_discrete" in data.files:
        raw = np.asarray(data["actions_discrete"], dtype=np.int64).reshape(-1, 1)
    else:
        return None
    if raw.ndim == 1:
        raw = raw.reshape(-1, 1)
...
def idle_rate(actions: np.ndarray) -> float:
    ...
    if arr.shape[0] <= 0 or arr.shape[1] < 4:
        return 0.0
    idle = (arr[:, 0] == 3) & (arr[:, 1] == 0) & (arr[:, 2] == 0) & (arr[:, 3] == 0)
    return float(np.mean(idle.astype(np.float32)))
...
def attack_rate(actions: np.ndarray) -> float:
    ...
    if arr.shape[0] <= 0 or arr.shape[1] < 4:
        return 0.0
    return float(np.mean((arr[:, 3] != 0).astype(np.float32)))
    ...
    try:
        with np.load(path, allow_pickle=False) as data:
            ...
            if obs.ndim != 2:
                ...
                return result
            if actions.ndim != 2:
                errors.append(f"actions must be 2D [N,A], got {actions.shape}")
                result["status"] = "FAIL"
                return result
            ...
            if actions.shape[1] != 4:
                errors.append(f"expected MultiDiscrete action dim 4, got {actions.shape[1]}")
